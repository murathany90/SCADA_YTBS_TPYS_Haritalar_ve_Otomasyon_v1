...
if ([string]::IsNullOrWhiteSpace([string]$auth.baseUrl) -or [string]::IsNullOrWhiteSpace([string]$auth.username) -or [string]::IsNullOrWhiteSpace([string]$auth.password)) { throw 'data/scada_auth.json Superset baseUrl/kullanici/parola alanlarini yerelde icermeli.' }
...
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'; $zip = Join-Path $dist "WebSCADA_0.1.0_$stamp.zip"
